                ...
                <form action="{{ route('sales.check-membership') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4" id="product-container">
                        @foreach ($products as $index => $product)
                            <div class="product-item bg-white rounded-lg shadow"
                                @if ($index >= 10) style="display: none;" @endif>
                                <img src="{{ asset('storage/' . $product->image) }}" class="w-full h-40 object-contain p-2"
                                    alt="{{ $product->name }}">
                                <div class="p-3">
                                    ...
                                    <div class="flex items-center mt-2 gap-2">
                                        <button type="button" class="w-6 h-6 bg-gray-100 rounded-full"
                                            onclick="changeQuantity(this, -1)">-</button>
                                        <input type="number"
                                            class="quantity-input w-12 text-center border rounded py-1 px-2"
                                            max="{{ $product->stock }}" onchange="updateTotals(this)" required>
                                        <button type="button" class="w-6 h-6 bg-gray-100 rounded-full"
                                            onclick="changeQuantity(this, 1)">+</button>
                                    </div>
                    ...
                    @if (count($products) > 10)
                        <div class="text-center" id="show-more-container">
                            <button type="button" id="show-more-btn"
                                class="border text-gray-700 px-4 py-1 rounded-full hover:bg-gray-50">
                    ...
                    <div class="fixed bottom-0 left-0 w-full p-4 bg-white border-t z-40">
                        <div class="max-w-7xl mx-auto flex justify-between items-center">
                            <div>
                                ...
                                <span class="text-sm text-gray-600 ml-4">Total: <span id="total-price"
                                        class="text-blue-600">Rp 0</span></span>
                            </div>
